...
export const RegisterMiniBoardDesktopEntryTable = ({
  ...
  <div className="mt-4 hidden overflow-hidden rounded-xl border border-zinc-800 md:block">
    <div className="grid grid-cols-[120px_160px_112px_1fr_120px_120px] border-b border-zinc-800 bg-zinc-950 px-3 py-2 text-xs font-medium uppercase tracking-wide text-zinc-400">
      <span>Data</span>
    ...
    <ul className="max-h-[460px] overflow-auto">
      {entries.map((entry) => {
        ...
        return (
          <li
            key={entry.id}
            className="grid grid-cols-[120px_160px_112px_1fr_120px_120px] items-center gap-2 border-b border-zinc-900 px-3 py-2 text-sm last:border-none"
          >
            <span className="text-zinc-300">{formatWorkDate(entry.work_date)}</span>
            <span className="min-w-0 truncate text-zinc-300">{timeRangeLabel}</span>
